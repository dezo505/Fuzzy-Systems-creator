...
public class MonotoneGaussPanel extends FuzzySetCreatorPanel{ //create Fuzzy sets in gaussian distribution shape

	private static final long serialVersionUID = -3570967345254528614L;
	
	private MonotoneGaussPanel self = this;
	...
	private MutableVar<Double> centerX;		//center of gaussian distribution			
	private MutableVar<Double> widthX;		//scalar of width of gaussian distribution	
	private MutableVar<Boolean> inverse;		//if true Inv f(x) = 1 - f(x)			
	private MutableVar<Boolean> leftSided;		//if true f(min) = mirr f(max)			
	private MutableVar<String> title;		//title of created set					
	
	private MutableVar<Integer>[] color;		//created set color		//int[3]
	
	@SuppressWarnings({ "unchecked" })
	public MonotoneGaussPanel(ProgramGUI programGUI, ProgramPanel panelParent, String panelTitle,
		...
		leftSided = new MutableVar<Boolean>(false);
	...
	public FuzzySet createFuzzySet() {
		FuzzySet result = FuzzyCreator.monotoneGaussFuzzySet(minX, maxX, widthX.getValue(), centerX.getValue(), leftSided.getValue(), inverse.getValue(), title.getValue(), 
	...
	public void addComponents(int width, int height) {
		...
		CheckBoxBool c2 = (new CheckBoxBool(leftSided, "Left sided",  width * 23/30, height * 9/20, width * 2/10, height * 1/20));
		c2.setBackground(programGUI.getSecondaryColor());
		c2.setForeground(programGUI.getTextColor());
		this.add(c2);
