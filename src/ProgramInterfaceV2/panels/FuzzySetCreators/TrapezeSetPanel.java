...
public class TrapezeSetPanel extends FuzzySetCreatorPanel{ //create Fuzzy sets in gaussian distribution shape

	private static final long serialVersionUID = -3570967345254528614L;
	
	private TrapezeSetPanel self = this;
	...
	private MutableVar<Double> middleX1;		
	private MutableVar<Double> middleX2;	
	...
	public TrapezeSetPanel(ProgramGUI programGUI, ProgramPanel panelParent, String panelTitle,
		...
		middleX1 = new MutableVar<Double>(minX + (maxX - minX) * 1/3);
		middleX2 = new MutableVar<Double>(minX + (maxX - minX) * 2/3);
		
		title = new MutableVar<String>("New trapeze set");
	...
	public FuzzySet createFuzzySet() {
		FuzzySet result = FuzzyCreator.trapezeFuzzySet(minX, maxX, edgeX1.getValue(), middleX1.getValue(), middleX2.getValue(), edgeX2.getValue(), inverse.getValue(),
	...
	public boolean isDataCorrect(){
		return edgeX1.getValue() < middleX1.getValue() && middleX1.getValue() < middleX2.getValue() && middleX2.getValue() < edgeX2.getValue();
	}
	...
	public void addComponents(int width, int height) {
		...
		ButtonTextField<Double> b3 =(new ButtonTextField<Double>(middleX1, "set", "middle 1", programGUI.getFrame(),
				width * 23/30, height * 5/25, width * 2/10, height * 1/20));
		b3.setColor(programGUI.getSecondaryColor(), programGUI.getTextColor());
		b3.addTo(this);
		
		ButtonTextField<Double> b4 = (new ButtonTextField<Double>(middleX2, "set", "middle 2", programGUI.getFrame(),
				width * 23/30, height * 7/25, width * 2/10, height * 1/20));
		...
		ButtonTextField<String> b5 = (new ButtonTextField<String>(title, "set", "Title", programGUI.getFrame(),
				width * 23/30, height * 9/25, width * 2/10, height * 1/20));
		b5.setColor(programGUI.getSecondaryColor(), programGUI.getTextColor());
		b5.addTo(this);
		
		CheckBoxBool c1 = (new CheckBoxBool(inverse, "Inversion",  width * 23/30, height * 11/25, width * 2/10, height * 1/20));
