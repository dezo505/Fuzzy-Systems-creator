...
public class ThresholdSetPanel extends FuzzySetCreatorPanel{ //create Fuzzy sets in gaussian distribution shape

	private static final long serialVersionUID = -3570967345254528614L;
	
	private ThresholdSetPanel self = this;
	...
	private MutableVar<Double> thresholdValue;		
	private MutableVar<Boolean> inverse;		
	private MutableVar<String> title;		//title of created set					//string
	
	private MutableVar<Integer>[] color;		//created set color		//int[3]
	
	@SuppressWarnings({ "unchecked" })
	public ThresholdSetPanel(ProgramGUI programGUI, ProgramPanel panelParent, String panelTitle,
		...
		thresholdValue = new MutableVar<Double>((maxX + minX)/2);
		inverse = new MutableVar<Boolean>(true);
		
		title = new MutableVar<String>("New threshold set");
	...
	public FuzzySet createFuzzySet() {
		FuzzySet result = FuzzyCreator.thresholdFuzzySet(minX, maxX, thresholdValue.getValue(), inverse.getValue(),
	...
	public void addComponents(int width, int height) {
		ButtonTextField<Double> b1 =(new ButtonTextField<Double>(thresholdValue, "set", "threshold", programGUI.getFrame(),
				width * 23/30, height * 1/25, width * 2/10, height * 1/20));
		b1.setColor(programGUI.getSecondaryColor(), programGUI.getTextColor());
		b1.addTo(this);
		
		CheckBoxBool c1 = (new CheckBoxBool(inverse, "Inversion",  width * 23/30, height * 3/25, width * 2/10, height * 1/20));
